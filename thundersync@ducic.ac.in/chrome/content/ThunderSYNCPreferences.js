...
FileName:   ThunderSYNCPreferences.js
Version:    V0.3.8
Date:       2012-11-17
...
var ThunderSYNCAddOnListener = {			
	
}

var ThunderSYNCPreferences = {	
	...
	{	
		...
		this.defaultPrefs = Components.classes["@mozilla.org/preferences-service;1"]
				...
				.getDefaultBranch("extensions.ThunderSYNC.");
		...
		AddonManager.addAddonListener(ThunderSYNCAddOnListener);	
		...
		{		
			...
			if ( this.contactDir == "") 
			{
				...
				window.open("chrome://Thunder-Sync/content/options.xul", "Thunder-Sync Options", "chrome");				
			}
			...
			if (document.title == "Thunder-Sync Options")
			{ 
	...
	shutdown: function()
	{
		...
		AddonManager.removeAddonListener(ThunderSYNCAddOnListener);
	},
...
window.addEventListener("load", function(e) { ThunderSYNCPreferences.startup(); }, false);
window.addEventListener("unload", function(e) { ThunderSYNCPreferences.shutdown(); }, false);
